...
% one cylindrical joint. Controller data is also loaded into the workspace.
% 
...
pos_base = [];
pos_top = [];
alpha_b = 7.5*deg2rad; % +- offset angle from 120 degree spacing on base  []
alpha_t = 30*deg2rad; % +- offset angle from 120 degree spacing on top [-30 60 120 180 240 300]
height = 2.0; % 2 meter height in home configuration
radius_b = 3.0; % base radius in meters
radius_t = 2.0; % top radius in meters
for i = 1:3
  % base points
  angle_m_b = (2*pi/3)* (i-1) - alpha_b;
  angle_p_b = (2*pi/3)* (i-1) + alpha_b;%[30 90 150 210 270 330]
...
% Load H_inf controller data
load StewartPlatformHinf;